...
import { MAINNET_TICKET_TOKEN_ADDRESS, PAY_TOKENS } from "../starknet/tokens.js";
...
function applyFeeBps(amount, feeBps) {
    const factor = 10000n + BigInt(feeBps);
    return ceilDiv(amount * factor, 10000n);
}
...
export function buildOrdersRouter(params) {
    ...
    router.post("/orders", async (req, res) => {
        ...
        const payTokenConfig = PAY_TOKENS[payToken];
        ...
        const buyAmount = 1000000000000000000n;
        ...
        const row = params.db.prepare("SELECT * FROM orders WHERE id = ?").get(orderId);
    ...
    router.get("/orders/:id", (req, res) => {
        ...
        const row = params.db.prepare("SELECT * FROM orders WHERE id = ?").get(id);
        if (!row) {
    ...
    router.post("/orders/:id/payment", async (req, res) => {
        ...
        if (row.status !== "awaiting_payment") {
            ...
            const current = params.db.prepare("SELECT * FROM orders WHERE id = ?").get(id);
        ...
        const updated = params.db.prepare("SELECT * FROM orders WHERE id = ?").get(id);
