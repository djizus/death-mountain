...
const ONE_TICKET = 1000000000000000000n;
    ...
    const tick = async () => {
        ...
        try {
            ...
            if (order.status === "awaiting_payment" && order.payment_tx_hash) {
                ...
                params.db
                    .prepare("UPDATE orders SET status = ?, updated_at = ?, paid_amount_raw = ?, last_error = NULL WHERE id = ?")
            ...
            if (order.status === "paid" && !order.fulfill_tx_hash) {
                ...
                const treasuryAddress = requireNormalizedAddress(params.config.STARKNET_TREASURY_ADDRESS, "treasury");
                ...
                if (!canFulfillOrder(ticketBalance, params.config.TICKET_RESERVE_MINIMUM)) {
                    console.error(`[worker] Cannot fulfill order: ${currentTickets} tickets <= ${params.config.TICKET_RESERVE_MINIMUM} minimum reserve`);
                    params.db
                        .prepare("UPDATE orders SET status = ?, updated_at = ?, last_error = ? WHERE id = ?")
                        .run("failed", Date.now(), "insufficient_ticket_balance", order.id);
                    return;
                }
                ...
                if (currentTickets < params.config.TICKET_RESERVE_TARGET) {
                    console.log(`[worker] Tickets (${currentTickets}) below target (${params.config.TICKET_RESERVE_TARGET}), triggering restock`);
                ...
                params.db
                    .prepare("UPDATE orders SET updated_at = ?, fulfill_tx_hash = ?, last_error = NULL WHERE id = ?")
            ...
            if (order.status === "fulfilling" && order.fulfill_tx_hash && !order.game_id) {
                ...
                if (result.executionStatus === "SUCCEEDED" && result.gameId !== null) {
                    ...
                    params.db
                        .prepare("UPDATE orders SET status = ?, updated_at = ?, game_id = ?, last_error = NULL WHERE id = ?")
                        .run("fulfilled", Date.now(), result.gameId, order.id);
                    return;
                }
                const error = result.revertReason ??
