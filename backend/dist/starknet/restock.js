...
import { MAINNET_TICKET_TOKEN_ADDRESS, PAY_TOKENS } from "./tokens.js";
...
const SELLABLE_TOKENS = ["ETH", "STRK", "USDC", "USDC_E", "SURVIVOR"];
function getAvnuBaseUrl() {
...
export function getRestockState() {
    return { ...restockState };
}
/**
...
export async function restockTicketsIfNeeded(params) {
    ...
    try {
        ...
        if (lordsBalance < lordsNeeded) {
            ...
            // Execute the swap
            const sellToLordsResult = await executeSwap(account, sellToLordsQuote, {
        ...
        const lordsToTicketResult = await executeSwap(account, finalQuote, {
            slippage: config.RESTOCK_SLIPPAGE,
            executeApprove: true
        }, { baseUrl: getAvnuBaseUrl() });
 ...
 */
export function canFulfillOrder(ticketBalance, minimumReserve) {
    const currentTickets = Number(ticketBalance / ONE_TICKET);
